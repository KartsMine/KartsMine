...
export function Onboarding() {
    ...
    function render() {
                        ...
                        ` : candidates.map(c => `
                            <tr style="border-bottom: 1px solid #f1f5f9; transition: background 0.1s;">
                                ...
                                <td style="padding: 16px 24px; text-align: right;">
                                    <button id="btn-action-${c.id}" class="btn-action-menu" data-id="${c.id}" style="color: #64748b; background: none; border: none; cursor: pointer; padding: 8px;">
        ...
        setTimeout(() => {
            ...
            const actionBtns = div.querySelectorAll('.btn-action-menu');
            actionBtns.forEach(btn => {
                btn.onclick = (e) => {
                    const id = Number(btn.dataset.id);
                    const action = prompt('Choose Action:\n1. Verify Documents (Completes 50%)\n2. Complete Training (100%)\n3. Delete Candidate');

                    if (action === '1') {
                        const candidate = store.candidates.find(c => c.id === id);
                        if (candidate) {
                            candidate.docStatus = 'success';
                            candidate.trainingProgress = 50;
                            candidate.status = 'Onboarding'; // ensure status
                            actions.saveSettings({}); // Trigger notify
                            window.showToast('Documents Verified', 'success');
                        }
                    } else if (action === '2') {
                        const candidate = store.candidates.find(c => c.id === id);
                        if (candidate) {
                            candidate.trainingProgress = 100;
                            candidate.status = 'Hired'; // Move to hired? Or keep in onboarding?
                            // Let's keep in Onboarding but 100%
                            actions.saveSettings({}); // Trigger notify
                            window.showToast('Training Completed', 'success');
                        }
                    } else if (action === '3') {
                        if (confirm('Are you sure you want to delete this candidate?')) {
                            store.candidates = store.candidates.filter(c => c.id !== id);
                            actions.saveSettings({}); // Trigger notify
                            window.showToast('Candidate Deleted', 'success');
                        }
                    }
                };
            });
